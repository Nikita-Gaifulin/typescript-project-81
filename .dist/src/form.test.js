...
test('throws error for non-existent field', () => {
    expect(() => {
        new HexletCode().formFor(template, {}, f => f.input('age')); // Поле 'age' отсутствует
    }).toThrowError('Field \'age\' does not exist in the template.');
});
test('throws error for non-existent field', () => {
    const html = new HexletCode().formFor(template, {}, f => f.submit('Save'));
